...
func NewInternalAPI(
	...
	forgetRoomConsumer := consumers.NewInputRoomForgetConsumer(base.ProcessContext, cfg, js, db)
	if err := forgetRoomConsumer.Start(); err != nil {
		logrus.WithError(err).Panic("failed to start user API forget room consumer")
	}
